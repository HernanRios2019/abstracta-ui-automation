...
- Add or modify Playwright fixtures, helper functions, or global setup/teardown files
- Update test documentation under `docs/QA/` or `docs/testing/`
- Configure Playwright settings (playwright.config.js/ts) with approval
- Execute test commands locally (`npx playwright test`) and review CI/CD pipeline results
- Create test data factories or fixtures
- Add custom Playwright fixtures or helper functions for reusable test logic
...
- Writing selectors or Playwright actions directly in test files (must use page objects)
...
#### Escalation Rules
> **CRITICAL**: Any exception to the rules or standards defined in this contract or `docs/QA` must be explicitly escalated to and approved by a human Technical Lead before proceeding.
...
> **Detailed technical standards, including Architecture (POM), Selectors, Waits, and Test Data are now maintained in [`docs/QA/README.md`](docs/QA/README.md).**
...
Refer to [`docs/QA/README.md`](docs/QA/README.md) for the allowed and recommended Playwright commands/actions (e.g., `page.click`, `page.fill`, `expect`).
...
5.  **Documentation Updated**: If the implementation changes patterns or adds new components, `DOCS/QA` must be updated.
...
Allowed tools: (These tools are analogous to Playwright's `page` object methods)
