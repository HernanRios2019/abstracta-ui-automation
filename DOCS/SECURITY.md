...
- Add or modify Playwright test fixtures, helpers, and global setup/teardown files
- Update test documentation under `docs/security/playwright/`
- Configure Playwright settings (playwright.config.js/ts) with approval
- Execute test commands locally and review CI/CD pipeline results
- Create test data factories or fixtures
- Add custom Playwright test fixtures or helper functions for reusable test logic
...
- Writing locators or Playwright actions directly in test files (must use page objects)
...
#### Context Sources
- docs/security/playwright/README.md
- docs/security/playwright/patterns.md
- docs/git.md
- playwright.config.js or playwright.config.ts
- tests/utils/test-helpers.js or tests/fixtures/custom-fixtures.js
- README.md
- package.json (for test scripts and dependencies)

#### Escalation Rules
> **CRITICAL**: Any exception to the rules or standards defined in this contract or `docs/security/playwright` must be explicitly escalated to and approved by a human Technical Lead before proceeding.
...
> **Detailed technical standards, including Architecture (POM), Locators, Waits, and Test Data are now maintained in [`docs/security/playwright/README.md`](docs/security/playwright/README.md).**
...
Refer to [`docs/security/playwright/README.md`](docs/security/playwright/README.md) for the allowed and recommended Playwright commands.
...
5.  **Documentation Updated**: If the implementation changes patterns or adds new components, `DOCS/security/playwright` must be updated.
...
# Tooling
This agent operates exclusively using the Chrome DevTools MCP server.
